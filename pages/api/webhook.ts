...
export default async function handler(
  ...
  try {
    ...
    const { cleanContent: processedContent, textContent } =
      processNewsletterContent(originalContent);
    const wordCount = textContent.split(/\s+/).filter(Boolean).length;
    const processingVersion = "2.7.0-with-content-processor";
    let cleanContent = processedContent;
    let enhancedProcessingVersion = processingVersion;
    ...
    try {
      ...
      enhancedProcessingVersion = `3.0.0-cleaner-${parseResult.metadata.processingVersion}`;
    ...
    const newsletter: Newsletter = {
      ...
      metadata: {
        processingVersion: enhancedProcessingVersion || processingVersion,
