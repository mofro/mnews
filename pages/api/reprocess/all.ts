  ...
  try {
    ...
    if (!confirmReprocessAll) {
      return res.status(400).json({
        error:
          "Must confirm bulk reprocessing by setting confirmReprocessAll: true",
    ...
    const processableNewsletters = (
      newsletters as unknown as Array<
        NewsletterMetadata & { rawContent?: string }
      >
    )
      .filter((newsletter): newsletter is ProcessableNewsletter => {
        return (
          "rawContent" in newsletter &&
    ...
    if (processableNewsletters.length === 0) {
      return res.status(400).json({
        ...
        suggestion:
          "Only newsletters created after content preservation can be reprocessed",
      });
    }

    logger.info(
      `Starting newsletter reprocessing for ${newsletters.length} newsletters...`,
    );
    ...
    for (const newsletter of processableNewsletters) {
      try {
        ...
        if (!newsletter.rawContent) {
          logger.warn(
            `Skipping newsletter ${newsletter.id} - no raw content available`,
          );
          continue;
        }
        ...
        const cleanContent = cleanedResult.cleanedContent;

        // Update the existing newsletter with new clean content
        const existingNewsletter = await NewsletterStorage.getNewsletter(
          newsletter.id,
        );
        ...
        const { processedAt: _, ...existingMetadata } =
          (existingNewsletter as any).metadata || {};
        const metadata = {
          ...existingMetadata,
          processedAt: new Date().toISOString(),
          parserVersion: "3.0.0-cleaner",
        };
        ...
        results.push({
          ...
          result: {
            ...
            processingInfo: {
              originalVersion:
                newsletter.metadata?.processingVersion || "unknown",
              newVersion: "3.0.0-cleaner",
            },
          },
        });

        successCount++;
        logger.info(`✅ Success: ${newsletter.id}`);
      } catch (error) {
    ...
    logger.info(
      `Bulk reprocessing complete: ${successCount} success, ${errorCount} errors`,
    );
    ...
    const compressionRatios = successfulResults
      .map((r: any) =>
        parseFloat(
          r.result.processingInfo?.compressionRatio?.replace("%", "") || "0",
        ),
      )
    ...
    const structureRecoveryCount = successfulResults.filter((r: any) =>
      r.result.processingInfo?.stepsExecuted?.includes(
        "recover-structure-enhanced",
      ),
    ).length;
