...
async function testHdel(useFirstImpl: boolean = true): Promise<boolean> {
  ...
  try {
    ...
    if (useFirstImpl) {
      ...
      console.log('Using first hdel implementation');
      deleted = await redis.hdel(testKey, field1, field2);
    } else {
      // Second implementation (directly calling client.hdel)
      console.log('Using second hdel implementation');
      // @ts-ignore - Accessing private member for testing
      deleted = await redis.client.hdel(testKey, field1, field2);
    }
    
    console.log('hdel result:', deleted);
    
    // Verify deletion
    const remaining = await redis.hgetall(testKey);
    console.log('Remaining fields:', remaining);
    
    return deleted === 2; // Should return 2 for 2 fields deleted
  } catch (error) {
    ...
    // Clean up
    // @ts-ignore - Accessing private member for testing
    await redis.client.del(testKey);
  }
}
